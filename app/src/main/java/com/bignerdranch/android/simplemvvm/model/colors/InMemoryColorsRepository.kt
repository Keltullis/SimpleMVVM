...
class InMemoryColorsRepository(
    ...
    /*override fun listenCurrentColor(): Flow<NamedColor> = callbackFlow {
        val listener:ColorListener ={
            // Передаём текущий цвет который пришел во flow
            trySend(it)
        }
        listeners.add(listener)

        // как только корутина закончится,этот метод выполнится
        awaitClose{
    ...
    */
    ...
    // Создаём свой flow
    // .flowOn заменяет withContext и перенаправляет операцию в другой поток
    // без флоуОн код будет выполняется в той короутине в котором был вызван
    // нужно будет вызвать функцию и на ней вызвать терминальный метод collect
